...
What follows is text that was present in the file, before modifying it.
...
*Emissions and Damages
 ...
 damfraceq(t) ..      DAMFRAC(t)     =E= (a2*TATM(t)**a3) ;
 omegaeq(t) ..	      OMEGA(t)       =E= cost1(t) * (MIU(t)**expcost2);
 dameq(t)..           DAMAGES(t)     =E= YGROSS(t) * DAMFRAC(t);
 abateeq(t)..         ABATECOST(t)   =E= YGROSS(t) * OMEGA(t);
...
mmat(t+1)..          MAT(t+1)       =E= 0.9942*MAT(t) + (E(t)*(5/3.666));
**NEW
...
tatmeq(t+1)..        TATM(t+1)      =E= -2.8672 + 0.8954*TATM(t) + 0.4622*log(MAT(t+1));
**NEW
...
**OLD
* yy(t)..              Y(t)           =E= YGROSS(t)*(1-OMEGA(t)-damfrac(t));
yy(t)..              Y(t)           =E= YGROSS(t)*(1-OMEGA(t))/(1+0.00265*TATM(t)**2);
**NEW
 cc(t)..              C(t)           =E= Y(t) - I(t);
...
* Produces a file "SICE_mDMH.csv" in the base directory
file results /SICE_mDMH.csv/; results.nd = 10 ; results.nw = 0 ; results.pw=20000; results.pc=5;
put results;
put /"Results of DICE-2016R model run using model SICE_mYMH.csv";
